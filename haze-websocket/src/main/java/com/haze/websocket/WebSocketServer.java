...
public class WebSocketServer {
    ...
    public void groupSending(String message){
        for (String name : connectedMap.keySet()){
            try {
                ...
                logger.error("[WebSocketServer] groupSending 信息发送失败, message={}, e={}", message, e);
            }
        }
    }
     ...
     * @param names 连接名称集合
     */
    public void groupSending(String message, Set<String> names){
        for (String name : names) {
            try {
                ...
                logger.error("[WebSocketServer] groupSending 信息发送失败, message={}, names={}, e={}", message, HazeStringUtils.join(names,","), e);
            }
        }
    }
     ...
     * @param name 连接名称
     */
    public void appointSending(String message, String name){
        try {
            connectedMap.get(name).session.getBasicRemote().sendText(message);
        }catch (Exception e){
            logger.error("[WebSocketServer] appointSending 信息发送失败, name={}, message={}, e={}", name, message, e);
        }
    }
