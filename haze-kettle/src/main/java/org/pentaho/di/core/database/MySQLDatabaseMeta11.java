    ...
    @Override public String getURL( String hostname, String port, String databaseName ) {
        if ( getAccessType() == DatabaseMeta.TYPE_ACCESS_ODBC ) {
            ...
            if ( Utils.isEmpty( port ) ) {
                return "jdbc:mysql://" + hostname + "/" + databaseName + "?useUnicode=true&serverTimezone=Asia/Shanghai&autoReconnect=true&useSSL=false";
            } else {
                return "jdbc:mysql://" + hostname + ":" + port + "/" + databaseName + "?useUnicode=true&serverTimezone=Asia/Shanghai&autoReconnect=true&useSSL=false";
            }
        }
    }
